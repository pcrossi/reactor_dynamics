...
Cenário: Cinética Pontual com inserção de reatividade senoidal em Reator Rápido.
Objetivo: Validar a estabilidade do solver numérico para sistemas rigidamente acoplados 
          (stiff) submetidos a oscilações periódicas de reatividade.
"""
...
def problema_1():
    ...
    2. Validação numérica pontual da evolução temporal (Tabela 3a).
    3. Detecção precisa de eventos de pico de potência (Tabela 3b).
    4. Geração de gráficos de resposta temporal em múltiplos ciclos (Figuras 2a e 2b).
    """
    print("\n" + "="*80)
    ...
    # O tempo de geração muito curto (1.0e-8 s) caracteriza um sistema de equações 
    # diferenciais ordinárias (EDO) rigidamente acoplado (stiff system).
    ...
    # Parâmetros da função de Reatividade Senoidal
    ...
    # Pontos de discretização temporal para comparação exata com a referência
    check_points = [10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0]
    
    # Dados de referência documentados em Ganapol (2013)
    ref_table = [
    ...
    # O método 'Radau' (implícito) é selecionado devido à rigidez extrema do sistema.
    # Tolerâncias rtol/atol são ajustadas para garantir precisão compatível com o benchmark.
    sol = solver.solve([0, 100], t_eval=check_points, method='Radau', rtol=1e-12, atol=1e-14)

    # Comparação ponto a ponto e cálculo do erro relativo
    ...
    for T_val, r_abs, tp_ref, np_ref in cases_3b:
        ...
        # Definição local da função de reatividade para o caso específico iterado
        ...
        # O objetivo matemático é identificar o instante preciso onde dN/dt = 0.
        def peak_event(t, y):
            # Acede ao método interno _rhs para extrair a derivada no instante t
            vals = sys_peak._rhs(t, y)
            return vals[0] # Retorna dN/dt
        
        peak_event.direction = -1  # Restringe a deteção apenas a cruzamentos descendentes (máximos locais)
        peak_event.terminal = True # Interrompe a integração imediatamente ao encontrar o evento
        
        # Simulação até T_val (cobre o primeiro semi-ciclo onde ocorre o primeiro pico)
        sol_peak = sys_peak.solve([0, T_val], events=peak_event, method='Radau', rtol=1e-12, atol=1e-14)

        # Extração dos resultados do evento detetado
        if sol_peak.t_events and len(sol_peak.t_events[0]) > 0:
            tp = sol_peak.t_events[0][0]         # Tempo do pico
            np_calc = sol_peak.y_events[0][0][0] # Amplitude da densidade no pico
        else:
            tp, np_calc = 0.0, 0.0
    ...
    # Simulação estendida para 1000s (abrangendo 10 ciclos) para visualização global.
    # O integrador (Radau) ajusta automaticamente o passo de tempo sem a necessidade de t_eval fixo.
    t_max_fig = 1000.0
    ...
    try:
        ...
        # Subplot 1: Detalhe linear do primeiro ciclo (0-100s)
        mask_2a = times <= 100.0
        plt.subplot(1, 2, 1)
